...
class _TermViewState extends State<_TermView> with SingleTickerProviderStateMixin {
  ...
  List<StudentModelP4> _sorted(List<StudentModelP4> src) {
    final f = src.where((s) {
      ...
      ..sort((a, b) => _agg(a.subjectsScore).compareTo(_agg(b.subjectsScore)));
    return f;
  }
  ...
  void _showDialog(BuildContext ctx, StudentModelP4 s, int pos) {
    final agg = _agg(s.subjectsScore);
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  _statChip('Total', _total(s.subjectsScore).toInt().toString()),
                  _statChip('Avg', _avg(s.subjectsScore).toStringAsFixed(1)),
                  _statChip('Agg', '$agg'),
                ],
              ),
            ),
            ...
            ConstrainedBox(
              ...
              child: ListView(
                ...
                children: s.subjectsScore.map((sub) {
  ...
  Widget _buildSummaryBar(List<StudentModelP4> students) {
    ...
    for (final s in students) {
      final d = _getDivision(_agg(s.subjectsScore));
  ...
  Widget _studentCard(BuildContext ctx, StudentModelP4 s, int i) {
    ...
    final div = _getDivision(agg);
    final divClr = _divColor(div);
    final total = _total(s.subjectsScore);
    final avg   = _avg(s.subjectsScore);
        ...
        onTap: allStudents.isEmpty
            ...
            : () {
                ...
                _downloadPdf(
                  ...
                  totalScore: (s) => _total(s.subjectsScore),
                  totalAgg:   (s) => _agg(s.subjectsScore),
                  avgScore:   (s) => _avg(s.subjectsScore),
...
// Public widgets
class TermBot extends StatelessWidget {
  final String model;
  final String schoolId;
  const TermBot({super.key, required this.model, required this.schoolId});
  ...
  Widget build(BuildContext context) => _TermView(
        ...
        analyticsDest: SubjectAnalysisBot(schoolId: schoolId, model: model),
        contributionDest: SubjectContributionBot(schoolId: schoolId, model: model),
      );
}

class TermMid extends StatelessWidget {
  final String model;
  final String schoolId;
  const TermMid({super.key, required this.model, required this.schoolId});
  ...
  Widget build(BuildContext context) => _TermView(
        ...
        analyticsDest: SubjectAnalysisMid(schoolId: schoolId, model: model),
        contributionDest: SubjectContributionMid(schoolId: schoolId, model: model),
      );
}

class TermEnd extends StatelessWidget {
  final String model;
  final String schoolId;
  const TermEnd({super.key, required this.model, required this.schoolId});
  ...
  Widget build(BuildContext context) => _TermView(
        ...
        analyticsDest: SubjectAnalysisEnd(schoolId: schoolId, model: model),
        contributionDest: SubjectContributionEnd(schoolId: schoolId, model: model),
      );
}