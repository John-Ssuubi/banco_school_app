...
import 'package:banco_mobile/HeadTeacher/AssessmentTerm3/subject_analysis_term3.dart';
import 'package:banco_mobile/HeadTeacher/AssessmentTerm3/subject_contribution_term3.dart';
        ...
        return pw.Column(
          ...
          children: [
            ...
            pw.Table.fromTextArray(
              ...
              data: s.subjectsScoreTerm3.map((sub) {
...
class _TermViewState extends State<_TermView> with SingleTickerProviderStateMixin {
  ...
  List<StudentModelP4> _sorted(List<StudentModelP4> src) {
    final f = src.where((s) {
      ...
      ..sort((a, b) => _agg(a.subjectsScoreTerm3).compareTo(_agg(b.subjectsScoreTerm3)));
    return f;
  }
  ...
  void _showDialog(BuildContext ctx, StudentModelP4 s, int pos) {
    final agg = _agg(s.subjectsScoreTerm3);
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  _statChip('Total', _total(s.subjectsScoreTerm3).toInt().toString()),
                  _statChip('Avg', _avg(s.subjectsScoreTerm3).toStringAsFixed(1)),
                  _statChip('Agg', '$agg'),
                ],
              ),
            ),
            ...
            ConstrainedBox(
              ...
              child: ListView(
                ...
                children: s.subjectsScoreTerm3.map((sub) {
  ...
  Widget _buildSummaryBar(List<StudentModelP4> students) {
    ...
    for (final s in students) {
      final d = _getDivision(_agg(s.subjectsScoreTerm3));
  ...
  Widget _studentCard(BuildContext ctx, StudentModelP4 s, int i) {
    ...
    final div = _getDivision(agg);
    final divClr = _divColor(div);
    final total = _total(s.subjectsScoreTerm3);
    final avg = _avg(s.subjectsScoreTerm3);
      ...
      _FabItem(
        ...
        onTap: allStudents.isEmpty
            ...
            : () {
                ...
                _downloadPdf(
                  context: context,
                  title: '${widget.model} — ${widget.examLabel} Assessment (Term 3)',
                  students: allStudents,
                  totalScore: (s) => _total(s.subjectsScoreTerm3),
                  totalAgg: (s) => _agg(s.subjectsScoreTerm3),
                  avgScore: (s) => _avg(s.subjectsScoreTerm3),
    ...
    return Column(
      ...
      children: [
              ...
              child: Row(
                ...
                children: [
                  ...
                  FloatingActionButton.small(
                    heroTag: '${item.label}_${widget.examLabel}_term3',
        ...
        FloatingActionButton(
          heroTag: 'main_${widget.examLabel}_term3',
...
// Public widgets for Term 3
class Term3Bot extends StatelessWidget {
  final String model;
  final String schoolId;
  const Term3Bot({super.key, required this.model, required this.schoolId});
  ...
  Widget build(BuildContext context) => _TermView(
        ...
        analyticsDest: SubjectAnalysisBotTerm3(schoolId: schoolId, model: model),
        contributionDest: SubjectContributionBotTerm3(schoolId: schoolId, model: model),
      );
}

class Term3Mid extends StatelessWidget {
  final String model;
  final String schoolId;
  const Term3Mid({super.key, required this.model, required this.schoolId});
  ...
  Widget build(BuildContext context) => _TermView(
        ...
        analyticsDest: SubjectAnalysisMidTerm3(schoolId: schoolId, model: model),
        contributionDest: SubjectContributionMidTerm3(schoolId: schoolId, model: model),
      );
}

class Term3End extends StatelessWidget {
  final String model;
  final String schoolId;
  const Term3End({super.key, required this.model, required this.schoolId});
  ...
  Widget build(BuildContext context) => _TermView(
        ...
        analyticsDest: SubjectAnalysisEndTerm3(schoolId: schoolId, model: model),
        contributionDest: SubjectContributionEndTerm3(schoolId: schoolId, model: model),
      );
}